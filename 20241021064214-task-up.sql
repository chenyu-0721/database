...
SELECT 
    ...
    cp.credit_amount, 
    cp.price
FROM "USER" u, "CREDIT_PACKAGE" cp
WHERE 
    (u.name = '王小明' AND cp.name = '14 堂組合包方案') OR
    (u.name = '王小明' AND cp.name = '21 堂組合包方案') OR
    (u.name = '好野人' AND cp.name = '14 堂組合包方案');
...
JOIN "USER" u ON c.user_id = u.id
JOIN "SKILL" s 
WHERE u.name = '肌肉棒子' AND s.name = '瑜伽';
...
JOIN "USER" u ON c.user_id = u.id
JOIN "SKILL" s 
WHERE u.name = 'Q太郎' AND s.name IN ('有氧運動', '復健訓練');
...
WHERE 
    cb.user_id = u.id AND 
    cb.course_id = c.id AND 
    u.name = '王小明' AND 
    c.user_id = (SELECT id FROM "USER" WHERE name = '李燕容');
...
WHERE 
    ...
    cb.course_id = c.id AND 
    u.name = '王小明' AND 
    c.user_id = (SELECT id FROM "USER" WHERE name = '李燕容') AND
    cb.status != '課程已取消';
-- 5-6. 查詢：計算用戶王小明的購買堂數，顯示須包含以下欄位： user_id , total。 (需使用到 SUM 函式與 Group By)
SELECT 
    u.id AS user_id, 
    SUM(cp.credit_amount) AS total
FROM "USER" u
...
WITH purchase_credits AS (
    SELECT 
        u.id AS user_id, 
        SUM(cp.credit_amount) AS total_credit
    FROM "USER" u
    JOIN "CREDIT_PURCHASE" cp ON u.id = cp.user_id
    WHERE u.name = '王小明'
    GROUP BY u.id
...
SELECT 
    pc.user_id, 
    pc.total_credit - COALESCE(uc.used_credit, 0) AS remaining_credit
FROM purchase_credits pc
LEFT JOIN used_credits uc ON pc.user_id = uc.user_id;
...
FROM "SKILL" s
JOIN "COACH_LINK_SKILL" cls ON s.id = cls.skill_id
...
SELECT 
    ...
    COUNT(cp.id) AS sales_quantity
FROM "CREDIT_PACKAGE" cp
JOIN "CREDIT_PURCHASE" cps ON cp.id = cps.credit_package_id
WHERE cps.purchase_at BETWEEN '2024-11-01' AND '2024-11-30'
GROUP BY cp.name;
...
-- 顯示須包含以下欄位： 預約會員人數
SELECT 
    COUNT(DISTINCT user_id) AS booked_members
FROM "COURSE_BOOKING"
WHERE 
